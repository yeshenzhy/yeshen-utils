export const types = {
    ...
    any: function testAny(_instance) {
...
export const FORMAT_REGEXPS = {
    ...
    "utc-millisec": (input) =>
        typeof input === "string" && parseFloat(input) === parseInt(input, 10) && !isNaN(input as any), // eslint-disable-line no-restricted-globals
    regex /* istanbul ignore next: not supporting regex right now */(input) {
...
FORMAT_REGEXPS["regexp"] = FORMAT_REGEXPS["pattern"] = FORMAT_REGEXPS.regex;
FORMAT_REGEXPS["ipv4"] = FORMAT_REGEXPS["ip-address"];

export function isFormat(input, format) {
